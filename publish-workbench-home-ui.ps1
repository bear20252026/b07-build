...
$source = Join-Path $root 'apps\desktop-shell\src-tauri\target\release\bundle\nsis\AI Work OS_0.1.4_x64-setup.exe'
$target = Join-Path $delivery 'AI Work OS_0.1.4_x64-setup.exe'
...
$manifest = [ordered]@{
  ...
  changes = @(
    ...
    'Release metadata, Tauri bundle metadata, desktop package version and installer filename now use semantic version 0.1.4 and a pre-build consistency check rejects mismatches',
