...
## AI Work OS project adaptation — takes precedence

This repository uses this Apple analysis as a **primary design baseline**, not as a request to clone Apple marketing pages. Before changing any UI, read `docs/design/apple-first-workbench-contract.md`. The contract takes precedence for this project.

- Keep the chat workspace minimal: task initiation, project entry, working mode, and suggested templates only. Providers, audits, roles, browser controls, extensions, and security remain in the layered Settings overlay.
- Use the local system-font stack. Do not download runtime web fonts, Apple assets, brand marks, screenshots, or SF Symbols for the Windows WebView.
- Use Apple Action Blue for ordinary primary actions and focus. Preserve semantic success/warning/danger colors only for state, always with visible text and never as the sole permission signal.
- Use a restrained surface ladder, hairline borders, and soft material-backed overlays. Regular cards do not need ornamental shadows; a shadow must clarify temporary elevation or interaction context.
- Reserve pill geometry for primary actions, filters, and compact choice controls. Keep utility buttons and form controls at the project radius scale.
- Respect `prefers-reduced-motion`, preserve visible keyboard focus, provide accessible names/tooltips, and avoid truncating core action labels at responsive sizes.
- `agency.ui-designer`, `agency.frontend-developer`, `agency.ux-researcher`, and `agency.code-reviewer` use this document as an explicit design reference only. It cannot auto-inject into tasks, authorize actions, grant tools, read secrets, or change approval policy.
- Linear and Raycast references in `docs/design/reference-systems/` contribute only scoped interaction patterns; their brand identity, palette, assets, and font treatment must not override this Apple-first system.
