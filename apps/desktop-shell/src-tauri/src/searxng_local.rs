...
fn query(request: &SearxngSearchRequest) -> Result<&str, &'static str> {
    ...
    if value.is_empty() || value.chars().count() > 2_000 {
        return Err("searxng-query-invalid");
    }
    Ok(value)
}
...
fn available_port() -> Result<u16, &'static str> {
    ...
    listener
        ...
        .map_err(|_| "searxng-port-unavailable")
}

fn resource_directory(app: &AppHandle) -> Result<PathBuf, &'static str> {
    let path = app
        .path()
        .resolve("research/searxng", BaseDirectory::Resource)
        .map_err(|_| "searxng-resource-unavailable")?;
    path.join("searx")
        .is_dir()
        .then_some(path)
        .ok_or("searxng-resource-unavailable")
}
...
fn runtime_directory(app: &AppHandle) -> Result<PathBuf, &'static str> {
    let directory = app
        .path()
        .app_data_dir()
        .map_err(|_| "searxng-runtime-directory-unavailable")?
        .join("searxng");
...
fn write_settings(app: &AppHandle, port: u16) -> Result<PathBuf, &'static str> {
    ...
    let nonce = SystemTime::now()
        ...
        .map_err(|_| "searxng-settings-unavailable")?
        .as_nanos();
...
async fn ensure_running(app: &AppHandle, state: &SearxngState) -> Result<u16, &'static str> {
    ...
    {
        let mut current = state.0.lock().map_err(|_| "searxng-state-unavailable")?;
        if let Some(runtime_port) = current.as_mut().and_then(|runtime| {
            runtime
                .child
                .try_wait()
                .ok()
                .and_then(|status| status.is_none().then_some(runtime.port))
        }) {
...
async fn stop_runtime(state: &SearxngState) -> Result<(), &'static str> {
    let runtime = state
        .0
        .lock()
        .map_err(|_| "searxng-state-unavailable")?
        .take();
...
fn response_from_json(
    ...
    value: Value,
    max_results: usize,
) -> Result<WebSearchResponse, &'static str> {
    let results = value
        .get("results")
        .and_then(Value::as_array)
        .ok_or("searxng-response-invalid")?;
    ...
    for result in results.iter().take(max_results) {
        ...
        if raw_content.chars().count() + segment.chars().count() > MAX_RAW_CONTENT_CHARS {
            break;
        }
        raw_content.push_str(&segment);
        sources.push(WebSearchSource {
            title: if title.is_empty() {
                "SearXNG 搜索来源".to_owned()
            } else {
                title
            },
            url,
    ...
    if raw_content.trim().is_empty() {
        return Err("searxng-no-results");
    }
    Ok(WebSearchResponse {
        query,
        summary: format!("本地 SearXNG 返回 {} 个来源。", sources.len()),
        raw_content,
        sources,
    })
}

pub async fn search_searxng_local_impl(
    app: &AppHandle,
    state: &SearxngState,
    ...
    let client = reqwest::Client::builder()
        ...
        .map_err(|_| "searxng-unavailable")?;
    let response = client
        .get(local_url(port, "/search"))
        .headers(loopback_headers())
        .query(&[("q", normalized_query.as_str()), ("format", "json")])
        .send()
        .await
        .map_err(|_| "searxng-request-failed")?;
    if !response.status().is_success() {
        return Err("searxng-request-rejected");
    }
    let value = response
        .json::<Value>()
        .await
        .map_err(|_| "searxng-response-invalid")?;
    response_from_json(normalized_query, value, max_results)
}

#[tauri::command]
pub async fn search_searxng_local(
    app: AppHandle,
    state: State<'_, SearxngState>,
    request: SearxngSearchRequest,
) -> Result<WebSearchResponse, &'static str> {
...
#[tauri::command]
pub async fn stop_searxng_local(state: State<'_, SearxngState>) -> Result<(), &'static str> {
    stop_runtime(&state).await
}

/// Returns state without starting Python or accessing the network, for the local diagnostics report.
#[tauri::command]
pub fn searxng_local_status(
    state: State<'_, SearxngState>,
) -> Result<SearxngLocalStatus, &'static str> {
    status(&state)
}
...
mod tests {
    ...
    fn projects_json_results_without_inventing_sources() {
        let response = response_from_json("NOVA".to_owned(), json!({ "results": [{ "title": "Example", "url": "https://example.com/post", "content": "可读正文" }] }), 8).expect("valid response");
