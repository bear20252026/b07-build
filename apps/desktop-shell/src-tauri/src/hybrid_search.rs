    ...
    let retained = query
        ...
        .filter(|item| {
            ...
            !item.is_empty()
                ...
                && !matches!(name.as_str(), "gclid" | "fbclid" | "mc_cid" | "mc_eid")
        })
        .collect::<Vec<_>>();
    if retained.is_empty() {
        normalized_base
    } else {
        format!("{}?{}", normalized_base, retained.join("&"))
    }
}
...
fn push_sources(
    target: &mut Vec<HybridSearchSource>,
    seen: &mut BTreeSet<String>,
    backend: &str,
    sources: impl IntoIterator<Item = (String, String)>,
) {
    for (title, url) in sources {
        if target.len() >= MAX_SOURCES {
            break;
        }
        ...
        target.push(HybridSearchSource {
...
fn receipt(backend: &str, result: &Result<usize, &'static str>) -> HybridBackendReceipt {
    match result {
        Ok(source_count) => HybridBackendReceipt {
            backend: backend.to_owned(),
            state: "succeeded",
            detail: "已返回原始研究内容和来源。".to_owned(),
            source_count: *source_count,
        },
        Err(error) => HybridBackendReceipt {
            backend: backend.to_owned(),
            state: "failed",
...
pub async fn search_hybrid(
    app: AppHandle,
    state: tauri::State<'_, searxng_local::SearxngState>,
    request: HybridSearchRequest,
) -> Result<HybridSearchResponse, &'static str> {
    let query = validate_query(&request.query)?.to_owned();
    let exa = web_search::search_web(web_search::WebSearchRequest {
        query: query.clone(),
        max_results: Some(MAX_SOURCES),
    });
    let international = last30days::run_last30days_research(
        app.clone(),
        last30days::Last30DaysRequest {
            query: query.clone(),
            mode: last30days::Last30DaysMode::Last30days,
        },
    );
    let chinese = last30days::run_last30days_research(
        app.clone(),
        last30days::Last30DaysRequest {
            query: query.clone(),
            mode: last30days::Last30DaysMode::Last30daysCn,
        },
    );
    let searxng = searxng_local::search_searxng_local_impl(
        &app,
        &state,
        searxng_local::SearxngSearchRequest {
            query: query.clone(),
            max_results: Some(MAX_SOURCES),
        },
    );
    let (exa_result, international_result, chinese_result, searxng_result) =
        tokio::join!(exa, international, chinese, searxng);
    ...
    let receipts = vec![
        receipt("exa", &exa_count),
        receipt("last30days", &international_count),
        receipt("last30days-cn", &chinese_count),
        receipt("searxng-local", &searxng_count),
    ];
...
mod tests {
    ...
    fn removes_tracking_parameters_and_deduplicates_sources() {
        assert_eq!(
            canonical_url("https://Example.com/Post?utm_source=x&tag=ai#top"),
            "https://example.com/post?tag=ai"
        );
        let mut sources = Vec::new();
        ...
        push_sources(
            ...
            [(
                "A".to_owned(),
                "https://example.com/post?utm_source=x".to_owned(),
            )],
        );
        push_sources(
            &mut sources,
            &mut seen,
            "last30days",
            [("B".to_owned(), "https://example.com/post".to_owned())],
        );
