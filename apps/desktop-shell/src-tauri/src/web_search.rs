...
fn validate_query(query: &str) -> Result<&str, &'static str> {
    ...
    if normalized.is_empty() || normalized.len() > 2_000 {
        return Err("web-search-query-invalid");
    }
    Ok(normalized)
}
...
fn result_text(body: &str) -> Option<String> {
    ...
    for line in body.lines() {
        ...
        let Some(content) = value.pointer("/result/content").and_then(Value::as_array) else {
            continue;
        };
        let text = content
            .iter()
...
fn sources_from_text(text: &str) -> Vec<WebSearchSource> {
    ...
    urls.into_iter()
        .take(MAX_RESULTS)
        .enumerate()
        .map(|(index, url)| WebSearchSource {
            title: format!("搜索来源 {}", index + 1),
            url,
        })
        .collect()
}
...
fn mcp_initialize_payload() -> Value {
    json!({
        ...
        "params": { "protocolVersion": "2025-03-26", "capabilities": {}, "clientInfo": { "name": "NOVA", "version": "0.1" } }
    })
}
...
pub async fn search_web(request: WebSearchRequest) -> Result<WebSearchResponse, &'static str> {
    ...
    let client = Client::builder()
        ...
        .user_agent("NOVA/0.1 WebSearch")
        .build()
        .map_err(|_| "web-search-unavailable")?;
    let initialized = client
        ...
        .send()
        .await
        .map_err(|_| "web-search-network-failed")?;
    if !initialized.status().is_success() {
        return Err("web-search-request-rejected");
    }
    ...
    if !response.status().is_success() {
        return Err(if response.status().as_u16() == 429 {
            ...
            "web-search-request-rejected"
        });
    }
    let body = response
        .text()
        .await
        .map_err(|_| "web-search-response-invalid")?;
        ...
        if let Ok((source, Some(page_text))) = result {
            if raw_content.len() < MAX_RAW_CONTENT_BYTES {
                ...
                raw_content.push_str(&format!(
                    ...
                    source.title,
                    source.url,
                    truncate_utf8(&page_text, remaining)
                ));
            }
        }
        if raw_content.len() >= MAX_RAW_CONTENT_BYTES {
            page_fetches.abort_all();
            break;
        }
...
mod tests {
    use super::{
        readable_page_text, result_text, sources_from_text, truncate_utf8, validate_query,
        MAX_RESULTS,
    };
    #[test]
    ...
    fn parses_json_rpc_content_when_remote_server_does_not_stream() {
        assert_eq!(
            result_text(
                r#"{"jsonrpc":"2.0","result":{"content":[{"type":"text","text":"内容"}]}}"#
            ),
            Some("内容".to_owned())
        );
    }
    #[test]
    fn retains_readable_text_and_never_splits_utf8() {
        assert_eq!(readable_page_text("<h1>标题</h1><p>正文</p>"), "标题\n正文");
