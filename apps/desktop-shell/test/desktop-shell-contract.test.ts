...
test('Android 候选构建链独立产出 NOVA aarch64 APK、universal AAB、临时签名、哈希清单与 SLSA provenance', () => {
  ...
  for (const expected of ['#!/bin/sh', 'set -eu', 'trap capture_evidence EXIT', 'nova-emulator-smoke.jks', 'apksigner_bin', 'startup_deadline=90', 'timeout 10 adb shell dumpsys activity activities', 'timeout 20 adb shell dumpsys window windows', "grep -q 'com.bear20252026.nova/.MainActivity'", "uiautomator dump /sdcard/nova-ui.xml >/dev/null 2>&1 || true"]) assert.ok(androidEmulatorSmokeScript.includes(expected), `Android emulator 冒烟脚本缺少：${expected}`);
