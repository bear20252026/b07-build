...
test('Android 候选构建链独立产出 NOVA aarch64 APK、universal AAB、临时签名、哈希清单与 SLSA provenance', () => {
  ...
  for (const expected of ['#!/bin/sh', 'set -eu', 'trap capture_evidence EXIT', 'nova-emulator-smoke.jks', 'apksigner_bin', "uiautomator dump /sdcard/nova-ui.xml >/dev/null 2>&1 || true", "grep -q 'com.bear20252026.nova'"]) assert.ok(androidEmulatorSmokeScript.includes(expected), `Android emulator 冒烟脚本缺少：${expected}`);
