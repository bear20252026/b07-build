...
test('Android 候选构建链独立产出 NOVA aarch64 APK、universal AAB、临时签名、哈希清单与 SLSA provenance', () => {
  for (const expected of [
    ...
    'Build Workbench static assets before Android generation',
    ...
    'Capture generated Android launch configuration',
  ...
  for (const expected of ['#!/bin/sh', 'set -u', 'trap finish EXIT', 'smoke-status.txt', 'nova-emulator-smoke.jks', 'apksigner_bin', 'timeout 30 adb wait-for-device',     'adb install --no-streaming -r -d',
    ...
    'install.txt',
    'package-path.txt', 'launch.txt', 'install_attempt=1', 'startup_deadline=120', 'render_elapsed=0', 'workbench-frame-not-rendered', 'verify-nova-android-frame.py', 'capture_timeout_diagnostics', 'nova-meminfo.txt', 'android-processes.txt', 'window-timeout.txt', 'nova-stack-request.txt', 'logcat-timeout.txt', 'adb shell kill -3', 'timeout 10 adb shell dumpsys activity activities', 'timeout 20 adb shell dumpsys window windows', "grep -q 'Splash Screen com.bear20252026.nova'", 'main-activity-splash-dismissed-and-workbench-frame-rendered', "grep -q 'com.bear20252026.nova/.MainActivity'", "uiautomator dump /sdcard/nova-ui.xml >/dev/null 2>&1 || true"]) assert.ok(androidEmulatorSmokeScript.includes(expected), `Android emulator 冒烟脚本缺少：${expected}`);
  assert.equal(existsSync(resolve(root, '.github/scripts/verify-nova-android-frame.py')), true, 'Android 冒烟必须包含首帧非空验证器');
