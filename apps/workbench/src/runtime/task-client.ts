...
export class HttpWorkbenchTaskClient implements WorkbenchTaskClient {
  ...
  async configureProviderSession(providerId: string, input: { displayName?: string; model?: string; baseUrl?: string; apiKey: string }): Promise<WorkbenchProviderConnection> {
    ...
    if (input.baseUrl !== undefined) {
      ...
      if (endpoint.protocol !== 'https:' || endpoint.username || endpoint.password || endpoint.search || endpoint.hash || (endpoint.port && endpoint.port !== '443')) throw new Error('连接地址必须是无认证信息的 HTTPS 标准端口地址');
    }
