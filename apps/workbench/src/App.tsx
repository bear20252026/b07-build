      ...
      <main className="workbench-main" style={androidRuntime ? { height: 'calc(100dvh - 64px)', borderRight: 0 } : undefined}>
        ...
        <section className="conversation-scroll" aria-label={messages.task.eventStreamAria}>
          <div className="conversation-frame">
            {workbenchSurface === 'chat-home' && <ChatHome activeProfile={activeProfile} authorityMode={authorityMode} canSaveAssistantArtifacts={!androidRuntime} connectedProviderCount={providerControl.connections?.length ?? 0} gatewayAttached={gatewayAttached} taskModelLabel={taskModelLabel} restoringProviderSession={providerControl.restoring} draftActive={Boolean(draft.trim())} directError={directSetupError ?? directConversations.error} directResponse={directConversations.activeConversation?.messages.at(-1)?.role === 'assistant' ? { output: directConversations.activeConversation.messages.at(-1)?.text ?? '', model: directConversations.activeConversation.messages.at(-1)?.model, complete: !directConversations.streaming } : undefined} messages={messages} activeConversation={directConversations.activeConversation} connections={providerControl.connections} discoveredModels={providerControl.discoveredModels} taskModelSelection={taskModelSelection} onSelectTaskModel={selectTaskModel} onPrepareSearchRetry={prepareSearchRetry} onOpenSearchSource={openSearchSource} onOpenContextBudget={() => setInspectorSurface('context-budget')} onOpenCheckpoints={() => setInspectorSurface('conversation-checkpoints')} onOpenUsageLedger={() => setInspectorSurface('usage-ledger')} onOpenSessionPerformance={() => setInspectorSurface('session-performance')} onBranchFromMessage={directConversations.branchFromMessage} onOpenModels={() => setActivePage('models')} onProfileChange={setActiveProfile} onSaveAssistantArtifact={(message) => void saveAssistantArtifact(message)} savingAssistantArtifactId={savingAssistantArtifactId} onSuggestion={useSuggestedGoal} profiles={profiles} />}
