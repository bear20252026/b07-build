...
export function ProjectBoard({ gatewayAttached, projects, selectedProjectId, projectTasks, activeTask, pending, error, onBackToChat, onCreate, onSelect, onAttachCurrentTask }: ProjectBoardProps) {
  const [title, setTitle] = useState(''); const [description, setDescription] = useState('');
  const selected = projects.find((project) => project.projectId === selectedProjectId);
  const canCreate = gatewayAttached && title.trim().length > 0 && !pending;
