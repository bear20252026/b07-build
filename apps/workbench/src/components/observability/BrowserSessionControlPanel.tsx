...
export function BrowserSessionSummaryCard({ localServiceReady, onOpen }: { localServiceReady: boolean; onOpen: () => void }) {
  const { sessions, error, pending, refresh } = useBrowserSessions(localServiceReady);
  ...
  return <section className="browser-session-summary-card" aria-label="浏览会话控制摘要">
    <div className="browser-session-summary-header"><div><span className="panel-eyebrow">BROWSER SESSION CONTROL</span><h2>浏览会话控制面</h2><p>记录目标主机、明确授权、暂停与结束；首轮不打开浏览器、不读取页面或 cookie，也不接管桌面。</p></div><button title="读取本机 本机能力服务 的脱敏浏览会话账本；不会启动浏览器或访问网页。" disabled={!localServiceReady || pending} onClick={refresh} type="button">{pending ? '读取中…' : '刷新摘要'}</button></div>
    {!localServiceReady && <p className="browser-session-inline-note">请先附着本机 本机能力服务。工作区首页不会显示或创建浏览会话。</p>}
    {localServiceReady && <div className="browser-session-summary-metrics"><div><span>已授权</span><strong>{counts.active}</strong></div><div><span>已暂停</span><strong>{counts.paused}</strong></div><div><span>会话总数</span><strong>{counts.total}</strong></div></div>}
    ...
    <button className="browser-session-open" title="进入三级浏览会话页面，显式创建或改变会话状态，并查看脱敏审计事件。" disabled={!localServiceReady} onClick={onOpen} type="button">打开浏览会话控制 <span aria-hidden="true">→</span></button>
  </section>;
}

export function BrowserSessionControlPage({ localServiceReady, onBack }: { localServiceReady: boolean; onBack: () => void }) {
  const { sessions, error, pending, refresh, setError, setPending, setSessions } = useBrowserSessions(localServiceReady);
  ...
  const openAudit = (sessionId: string): void => {
    if (!localServiceReady || auditPending) return;
