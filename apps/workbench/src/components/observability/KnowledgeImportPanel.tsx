...
/**
 * 二级设置中的手动知识导入入口。用户只能主动粘贴一段文本，选择范围后由 本机能力服务 建立可恢复摘要收据。
 * 不显示文件浏览器、不接受路径、不自动读取剪贴板，也不把正文重新回显到导入历史。
 */
export function KnowledgeImportPanel({ localServiceReady }: { localServiceReady: boolean }) {
  ...
  }, [localServiceReady, workspaceId]);

  const submit = (): void => {
    const trimmedTitle = title.trim();
    const trimmedText = text.trim();
    const storageBudgetBytes = Math.round(budgetMb * 1024 * 1024);
    if (!localServiceReady || pending || !workspaceId || !trimmedTitle || !trimmedText || !Number.isSafeInteger(storageBudgetBytes) || storageBudgetBytes < 1) return;
    setPending(true);
    setError(undefined);
  ...
  return (
    <section className="knowledge-import-panel" aria-label="手动知识导入">
      <header><div><span className="panel-eyebrow">EXPLICIT KNOWLEDGE IMPORT</span><h2>导入一段可审查文本</h2><p>只导入你主动粘贴的内容。系统保存摘要、范围和预算收据；不会扫描本地文件、读取剪贴板或上传内容。</p></div><span className={`knowledge-import-status${localServiceReady ? ' ready' : ''}`}>{localServiceReady ? '本机能力服务 已就绪' : '先附着 本机能力服务'}</span></header>
      {!localServiceReady && <p className="knowledge-import-note">此面板需要已附着的本机 本机能力服务；不会替你启动其他服务。</p>}
      {localServiceReady && <div className="knowledge-import-form">
        ...
        <div className="knowledge-import-footer"><span>{text.length.toLocaleString()} / 1,500,000 字符 · 正文只在本次明确提交时发送至本机 本机能力服务。</span><button title="创建包含内容摘要、范围、字节预算与分块数量的本地导入收据；不会自动调用模型。" disabled={!localServiceReady || !workspaceId || !title.trim() || !text.trim() || pending} type="button" onClick={submit}>{pending ? '正在建立收据…' : '导入并建立收据'}</button></div>
      </div>}
