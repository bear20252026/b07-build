 ...
 */
export function ProceduralCompanionStage({ compact = false, enabled }: { compact?: boolean; enabled: boolean }) {
  const style = compact ? { minHeight: 96 } : undefined;
  if (!enabled) return <div aria-label="Companion 角色呈现关闭" className="procedural-companion-stage" style={style}><span>角色呈现关闭</span></div>;
  return <div aria-label="本地程序化 3D Companion 预览" className="procedural-companion-stage" style={style}>
