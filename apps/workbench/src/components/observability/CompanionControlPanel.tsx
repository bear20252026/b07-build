...
function visualModeLabel(mode: CompanionPreferencesV1['visualMode']): string {
  return mode === 'three-dimensional' ? '3D / VRM 舞台' : '2D 动态玩偶';
}
...
export function CompanionSummaryCard({ preferences, onOpen }: { preferences: CompanionPreferencesV1; onOpen: () => void }) {
  return <section className="companion-summary-card" aria-label="Companion Agent 摘要">
    <div className="companion-summary-heading"><div><span className="panel-eyebrow">COMPANION AGENT</span><h2>3D 桌面角色</h2><p>默认启用视觉角色，现有 2D 玩偶仍可使用。角色只是本地视觉状态，不会自动取得模型、文件、浏览器、桌面或管理员权限。</p></div><span className={`companion-state-pill${preferences.visualEnabled ? ' enabled' : ' disabled'}`}>{preferences.visualEnabled ? '视觉已启用' : '视觉已关闭'}</span></div>
    <div className="companion-summary-metrics"><div><span>当前呈现</span><strong>{preferences.visualEnabled ? visualModeLabel(preferences.visualMode) : '完全关闭'}</strong></div><div><span>语音</span><strong>{preferences.voiceEnabled ? '已单独启用' : '默认关闭'}</strong></div><div><span>高影响能力</span><strong>全部关闭</strong></div></div>
...
export function CompanionControlPage({ gatewayAttached, preferences, onBack, onUpdate }: {
  ...
  return <section className="page-stack companion-control-page"><div className="page-heading companion-page-heading"><div><span>CAPABILITIES / COMPANION</span><h1>Companion Agent</h1><p>三级设置页。3D 角色默认作为本地视觉能力启用；关闭后不会加载角色舞台。语音与其它能力独立于角色视觉开关。</p></div><button title="返回二级扩展与能力页面。" onClick={onBack} type="button">← 返回扩展与能力</button></div>
    <section className="companion-control-hero"><div><span className="panel-eyebrow">VISUAL PRESENCE</span><h2>{visualStatus}</h2><p>{preferences.visualEnabled ? '当前默认使用 3D / VRM 舞台模式。首阶段不捆绑任何未经审查的 VRM 模型文件；如没有本地模型资产，会清晰显示等待导入状态。' : '角色视觉已完全关闭。现有任务、Provider、文件、审计和设置功能不会受到影响。'}</p></div><label className="companion-switch"><input aria-label="启用 Companion 角色视觉" checked={preferences.visualEnabled} onChange={(event) => onUpdate({ visualEnabled: event.target.checked })} type="checkbox" /><span aria-hidden="true" /><strong>{preferences.visualEnabled ? '角色已开启' : '角色已关闭'}</strong></label></section>
    <section className="companion-visual-settings"><div><span className="panel-eyebrow">RENDERER</span><h2>角色呈现方式</h2><p>2D 与 3D 共用受控角色状态；切换只影响界面呈现，不会切换 Agent、模型或执行权限。</p></div><div className="companion-mode-options"><button aria-pressed={preferences.visualMode === 'three-dimensional'} className={preferences.visualMode === 'three-dimensional' ? 'active' : ''} disabled={!preferences.visualEnabled} onClick={() => onUpdate({ visualMode: 'three-dimensional' })} title="选择 3D / VRM 舞台模式；当前仅准备渲染边界和状态投影。" type="button"><span>3D</span><strong>VRM 舞台</strong><small>眨眼、注视与待机动作的受控扩展点</small></button><button aria-pressed={preferences.visualMode === 'two-dimensional'} className={preferences.visualMode === 'two-dimensional' ? 'active' : ''} disabled={!preferences.visualEnabled} onClick={() => onUpdate({ visualMode: 'two-dimensional' })} title="保留现有 2D 动态玩偶，不会改变任务或模型。" type="button"><span>2D</span><strong>现有动态玩偶</strong><small>Orbit、Mori、Pixel、Sage 继续可用</small></button></div></section>
