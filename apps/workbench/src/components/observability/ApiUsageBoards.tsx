...
function useApiUsage(enabled: boolean, includeReceipts: boolean) {
  ...
  useEffect(() => { refresh(); // 仅在显式进入相应二/三级页面且 Gateway 已附着时拉取。
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [enabled, includeReceipts]);
  return { summary, receipts, error, pending, refresh };
}

export function ApiUsageSummaryCard({ gatewayAttached, onOpen }: { gatewayAttached: boolean; onOpen: () => void }) {
  const { summary, error, pending, refresh } = useApiUsage(gatewayAttached, false);
  return <section className="api-usage-summary-card" aria-label="API 使用摘要">
    <div className="api-usage-summary-header"><div><span className="panel-eyebrow">LOCAL API USAGE</span><h2>第三方 API 使用摘要</h2><p>仅统计本机 Gateway 已完成的受控推理调用，不读取供应商账户账单。</p></div><button title="读取当前本机 Gateway 的只读使用摘要；不会请求模型、价格或供应商账户。" disabled={!gatewayAttached || pending} onClick={refresh} type="button">{pending ? '读取中…' : '刷新摘要'}</button></div>
    {!gatewayAttached && <p className="api-usage-inline-note">请先附着本机 Gateway。首页不会显示 API 使用数据。</p>}
    {gatewayAttached && !error && <div className="api-usage-summary-metrics"><div><span>完成调用</span><strong>{summary?.totalCalls ?? '—'}</strong></div><div><span>平均延迟</span><strong>{summary ? formatLatency(summary.totalLatencyMs, summary.totalCalls) : '—'}</strong></div><div><span>最近活动</span><strong>{formatTime(summary?.latestRecordedAt)}</strong></div></div>}
    {gatewayAttached && <div className="api-usage-unknown"><strong>Token 与成本：未报告</strong><span>当前 Driver 未统一提供供应商 billing usage，因此不会把字符数或本地估算伪装为 token 或账单。</span></div>}
    {error && <p className="api-usage-error" role="alert">{error}</p>}
    <button className="api-usage-open" title="进入三级 API 使用审计页，查看 Provider、模型聚合和单次脱敏收据。" disabled={!gatewayAttached} onClick={onOpen} type="button">查看 API 使用审计 <span aria-hidden="true">→</span></button>
  </section>;
}

export function ApiUsageAuditPage({ gatewayAttached, onBack }: { gatewayAttached: boolean; onBack: () => void }) {
  const { summary, receipts, error, pending, refresh } = useApiUsage(gatewayAttached, true);
  ...
  return <section className="page-stack api-usage-audit-page"><div className="page-heading api-usage-page-heading"><div><span>RUN RECORDS / API USAGE</span><h1>API 使用审计</h1><p>三级只读页面。按 Provider 与模型查看 Gateway 已完成的调用收据；没有 prompt、输出、API key、端点、真实账单或自动刷新价格。</p></div><div><button title="返回二级运行记录页面。" onClick={onBack} type="button">← 返回运行记录</button><button title="重新读取本机 Gateway 的只读用量投影；不会调用任何第三方 API。" disabled={!gatewayAttached || pending} onClick={refresh} type="button">{pending ? '读取中…' : '刷新'}</button></div></div>
    {!gatewayAttached && <p className="api-usage-inline-note">请先附着本机 Gateway 后查看本机使用账本。</p>}
    {gatewayAttached && <><div className="api-usage-filters"><label>Provider<select title="筛选本机收据，不会切换或调用 Provider。" value={provider} onChange={(event) => { setProvider(event.target.value); setModel('all'); }}><option value="all">全部 Provider</option>{providers.map((value) => <option key={value} value={value}>{value}</option>)}</select></label><label>模型<select title="筛选本机收据，不会切换或调用模型。" value={model} onChange={(event) => setModel(event.target.value)}><option value="all">全部模型</option>{models.map((value) => <option key={value} value={value}>{value}</option>)}</select></label><span>仅已完成调用 · token/cost 未报告</span></div>
