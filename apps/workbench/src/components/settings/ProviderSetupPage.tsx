    ...
    <div className="provider-setup-page provider-setup-page--focused provider-setup-page--three-step">
      ...
      <section className="provider-onboarding provider-onboarding--compact provider-three-step" aria-label="Third-party API setup">
        ...
        <div className="provider-preset-grid provider-preset-grid--compact provider-preset-grid--simple">
          {PRESETS.map((item) => <button key={item.id} type="button" className={`provider-preset${!customMode && providerId === item.id ? ' active' : ''}`} onClick={() => choosePreset(item.id)}><strong>{item.title}</strong><small>{item.description}</small></button>)}
          <button type="button" className={`provider-preset provider-preset--custom${customMode ? ' active' : ''}`} onClick={chooseCustom}><strong>自定义 API</strong><small>OpenAI 或 Anthropic-compatible</small></button>
        </div>
        ...
        <div className="provider-onboarding-form provider-onboarding-form--simple">
          ...
          {!customMode && <button className="provider-model-adjust" type="button" onClick={() => setAdvancedOpen((open) => !open)}>{advancedOpen ? '收起模型调整' : `调整模型或名称（可选）`}</button>}
        ...
        <div className="provider-submit-row"><button className="provider-onboarding-submit" type="button" disabled={!gatewayAttached || !apiKey.trim() || (customMode && (!baseUrl.trim() || !model.trim() || !displayName.trim())) || isSubmitting} onClick={submit}>{isSubmitting ? '正在保存…' : '保存连接'}</button><button type="button" className="provider-next-link" onClick={onManageConnections}>查看已连接模型 →</button></div>
      </section>
