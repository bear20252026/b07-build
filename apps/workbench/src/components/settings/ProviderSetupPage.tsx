...
/** 三步线性连接页：选择 → 最少字段 → 连接并测试；按需启动的固定本机 Gateway 承载唯一一次显式模型探测。 */
  ...
  return (
    <div className="provider-setup-page provider-setup-page--focused provider-setup-page--three-step">
      <header className="settings-page-header provider-setup-header">
        <div><span>THIRD-PARTY API</span><h1>三步连接模型</h1><p>填写密钥后直接连接并测试；应用会按需启动本机 Gateway，第三方响应只回到本机工作台。</p></div>
      ...
      <section className="provider-onboarding provider-onboarding--compact provider-three-step" aria-label="Third-party API setup">
        ...
        <div className="provider-preset-grid provider-preset-grid--compact provider-preset-grid--simple">
          {PRESETS.map((item) => <button key={item.id} type="button" title={`选择 ${item.title}；默认模型 ${item.defaultModel}。`} className={`provider-preset${!customMode && providerId === item.id ? ' active' : ''}`} onClick={() => choosePreset(item.id)}><strong>{item.title}</strong><small>{item.description}</small></button>)}
        ...
        <div className="provider-onboarding-form provider-onboarding-form--simple">
          {customMode && <>
            ...
            <label><span>HTTPS Base URL</span><input value={baseUrl} inputMode="url" maxLength={512} onChange={(event) => setBaseUrl(event.target.value)} placeholder={protocol === 'openai-compatible' ? 'https://api.example.com/v1' : 'https://api.example.com'} /><small>你的提示会发送到此服务。请填写可访问的 HTTPS Base URL。</small></label>
        ...
        <div className="onboarding-step onboarding-step--final"><span>3</span><div><strong>连接并测试</strong><p>本次点击会按需启动本机 Gateway，将密钥仅写入当前会话内存，并发起一次模型列表探测；不会自动发送聊天内容或保存密钥。</p></div></div>
        <div className="provider-submit-row"><button className="provider-onboarding-submit" title="按需启动固定本机 Gateway，将连接仅保存到当前会话内存，并发起一次模型列表测试。" type="button" disabled={!apiKey.trim() || (customMode && (!baseUrl.trim() || !model.trim() || !displayName.trim())) || isSubmitting || attachingGateway} onClick={submit}>{isSubmitting ? '正在连接并测试…' : attachingGateway ? '正在准备 Gateway…' : '连接并测试'}</button><button type="button" title="前往已连接模型页，查看测试结果或发送一次文本请求。" className="provider-next-link" onClick={onManageConnections}>查看已连接模型 →</button></div>
      </section>
