...
assertNoMatch(workbench, /(?:from\s+['"]node:|\bDatabaseSync\b|\bchild_process\b|from\s+['"]@awo\/(?:agent-runtime|knowledge-workflow))/u, 'workbench-layer-leak');

// 领域包与跨语言控制面不允许反向依赖 application 层。
assertNoMatch(domain, /(?:from\s+['"](?:\.\.\/)*apps\/|@awo\/workbench)/u, 'domain-reverse-app-dependency');
